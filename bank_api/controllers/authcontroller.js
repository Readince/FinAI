...
if (!tckn || !password) return res.status(400).json({ success: false, message: "Eksik bilgi" });

const result = await login(tckn, password);
if (!result) return res.status(401).json({ success: false, message: "TCKN veya şifre hatalı" });
