...
function randTckn() {
  let t;
  do { t = generateValidTCKN(); } while (usedTckn.has(t));
  usedTckn.add(t);
  return t;
}
...
function randAccountNo() {
  let acc;
  do { acc = fakerTR.string.numeric(10); } while (usedAcct.has(acc));
  usedAcct.add(acc);
  return acc;
}
  ...
  try {
    ...
    for (let start = 0; start < total; start += CUST_BATCH_SIZE) {
      ...
      for (let i = start; i < end; i++) {
        ...
        // Kişisel bilgiler
        const birth_date  = fmtDate(fakerTR.date.birthdate({ min: 18, max: 80, mode: 'age' })); // DATE -> 'YYYY-MM-DD'
        ...
        const email  = safe(fakerEN.internet.email({ firstName: first_name, lastName: last_name }), 100); // EN fallback
        ...
        custParams.push(
          national_id, first_name, last_name, phone, email, address, branchId,
      ...
      `;
      const { rows: insertedCustomers } = await client.query(custInsert, custParams);
      ...
      for (const c of insertedCustomers) {
        ...
        for (let j = 0; j < accountCount; j++) {
          ...
          const balance   = Number(fakerTR.finance.amount({ min: 0, max: 100000, dec: 2 }));
          ...
          accParams.push(
            c.id, currency, type, balance, interest, accountNo, c.branch_id
          );
        }
      }
