export const maskTc = (tc) =>
  typeof tc === "string" ? tc.replace(/^(\d{0,9})(\d{2})$/, (_m, a, b) => `${"*".repeat(a.length)}${b}`) : tc;
...
export const sanitizePIIText = (text) => {
  ...
  // TCKN: 11 hane
  text = text.replace(/\b(\d{2})\d{7}(\d{2})\b/g, (_, a, b) => `${a}${"*".repeat(7)}${b}`);

  // Türkiye cep: +90 5xxxxxxxxx / 05xxxxxxxxx (mask basitleştirilmiş)
  text = text
    .replace(/\+90\s*5\d{2}\s*\d{3}\s*\d{2}\s*\d{2}\b/g, (m) => m.replace(/\d/g, "*"))
