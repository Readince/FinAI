...
if (!payload?.jti) return res.status(401).json({ success: false, message: "Token geçersiz" });

if (await isBlacklisted(payload.jti)) {
return res.status(401).json({ success: false, message: "Oturum sonlandırıldı" });
}
...
return res.status(401).json({ success: false, message: "Token doğrulanamadı" });
}
}